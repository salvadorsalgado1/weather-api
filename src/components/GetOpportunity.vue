             ...
             <div v-if="!show" class="col m12">
                 <div class="card">
                     <div class="card-content">
                        <UhOh/>
                        <p>Check out <router-link :to="{name:'Curiosity'}">Curiosity</router-link> or <router-link :to="{name:'Spirit'}">Spirit</router-link> in the meantime!</p>
                     </div>
                 </div>
             </div>
             <div v-if="show">
...
mounted(){
    ...
    Axios.get(`https://api.nasa.gov/mars-photos/api/v1/rovers/opportunity/photos?earth_date=${current}&api_key=${process.env.VUE_APP_NASA_KEY}`)
    .then((response)=>{
