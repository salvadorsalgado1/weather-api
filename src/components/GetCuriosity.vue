             ...
             <div v-if="!show" class="col m12">
                 <div class="card">
                     <div class="card-content">
                        <UhOh/>
                        <p>Check out <router-link :to="{name:'Spirit'}">Spirit</router-link> or 
                        <router-link :to="{name:'Opportunity'}">Opportunity</router-link> in the meantime!</p>
                     </div>
                 </div>
             </div>
             <div v-if="show">
...
methods:{
    getMore(){
        ...
        this.display = this.results.slice(0,5).map((result)=>{
...
mounted(){
    ...
    Axios.get(`https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?earth_date=${current}&api_key=${process.env.VUE_APP_NASA_KEY}`)
    .then((response)=>{
